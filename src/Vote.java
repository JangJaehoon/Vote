    ...
    public static void main(String[] args) {
        ...
        for(int i=1; i <= total; i++){
            ...
            System.out.printf("[기호:1] 이재명: %05.02f%c, (투표수: %d)\n",
                    ((double) candidate[0] / (double)total) * 100.0f,'%', candidate[0]); // %d 에 들어갈 값
            System.out.printf("[기호:2] 윤석열: %05.02f%c, (투표수: %d)\n",
                    ((double) candidate[1] / (double)total) * 100.0f,'%', candidate[1]); // %d 에 들어갈 값
            System.out.printf("[기호:3] 심상정: %05.02f%c, (투표수: %d)\n",
                    ((double) candidate[2] / (double)total) * 100.0f,'%', candidate[2]); // %d 에 들어갈 값
            System.out.printf("[기호:4] 안철수: %05.02f%c, (투표수: %d)\n",
                    ((double) candidate[3] / (double)total) * 100.0f,'%', candidate[3]); // %d 에 들어갈 값
